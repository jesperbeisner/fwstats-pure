...
final class LoginControllerTest extends AbstractTestCase
{
    ...
    public function test_post_request_without_username(): void
    {
        ...
        self::assertSame($session->getFlash(FlashEnum::ERROR), ['text.username-and-password-cant-be-empty']);
    }
    ...
    public function test_post_request_with_non_existing_user(): void
    {
        ...
        self::assertSame(302, $response->statusCode);
        self::assertSame('/login', $response->location);
        self::assertSame($session->getFlash(FlashEnum::ERROR), ['text.user-with-username-does-not-exist']);
    }
    ...
    public function test_post_request_with_wrong_password(): void
    {
        ...
        self::assertSame(302, $response->statusCode);
        self::assertSame('/login', $response->location);
        self::assertSame($session->getFlash(FlashEnum::ERROR), ['text.password-does-not-match']);
    }
    ...
    public function test_post_request(): void
    {
        ...
        self::assertSame($session->getFlash(FlashEnum::SUCCESS), ['text.login-success']);
    }
    ...
    public function test_post_request_with_redirect(): void
    {
        ...
        $this->getContainer()->set(Request::class, $request);
        $session = $this->getContainer()->get(SessionInterface::class);
        $session->set('security_request_uri', '/test');
