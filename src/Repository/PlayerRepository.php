...
final class PlayerRepository extends AbstractRepository implements ResetActionFreewarInterface
{
    ...
    {
        $sql = <<<SQL
            SELECT players.world, players.player_id, players.name, players.xp, players.soul_xp, players.total_xp, players_xp_history.start_xp, players_xp_history.end_xp, players_xp_history.end_xp - players_xp_history.start_xp AS xp_changes
            FROM players
        ...
        /** @var array<array{world: string, player_id: int, name: string, xp: int, soul_xp: int, total_xp: int, start_xp: int, end_xp: int, xp_changes: int}> $result */
        $result = $this->database->select($sql, [
